...
class InstagramLinksController extends Controller
{
    public function index()
    {
        $instagramLinks = InstagramLinks::orderBy('isBlocked', 'asc')
            ->orderBy('id', 'desc')
            ->paginate(10);

        return view('instagram.index', compact('instagramLinks'));
    }
    ...
    public function store(Request $request)
    {
        ...
        DB::beginTransaction();
        try {
            $totalLinks = InstagramLinks::count();
            $newLink = new InstagramLinks();
    ...
    {
        $link = instagramLinks::findOrFail($id);
    ...
    {
        $instagramLinks = instagramLinks::where('isDeleted', false)
    ...
    public function singleLink(Request $request)
    {
        ...
        $link = instagramLinks::where('id', $request->id)->first();
        if (!$link) {
            return response()->json([
                'success' => false,
                'status' => 404,
