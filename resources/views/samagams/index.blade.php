                        ...
                        <div class="card-header d-flex align-items-center justify-content-between flex-wrap">
                            ...
                            <div class="dropdown">
                                ...
                                <ul class="dropdown-menu">
                                    ...
                                    <li><a class="dropdown-item" href="/samagam/viewed">Approved</a></li>
                                    <li><a class="dropdown-item" href="/samagam/unviewed">Pending</a></li>
                                </ul>
                            </div>
                        </div>
                                ...
                                <tbody>
                                    @foreach ($samagams as $t)
                                        <tr>
                                            ...
                                            <td>
                                                <!-- View Modal Button -->
                                                <button title="View" class="btn btn-sm btn-primary view-samagam-btn me-2"
                                                    data-bs-toggle="modal" data-bs-target="#samagamModal"
                                                    data-organizer="{{ $t->organizerName }}" data-address="{{ $t->address }}"
                                                    data-phone="{{ $t->phone }}" data-email="{{ $t->email }}"
                                                    data-start="{{ $t->startDate }}" data-end="{{ $t->endDate }}"
                                                    data-details="{{ $t->details }}" data-map="{{ $t->mapLink }}">
                                                    <i class="bi bi-eye"></i>
                                                </button>

                                                <!-- Mark as Viewed -->
                                                @if (!$t->isViewed)
                                                    <form action="{{ route('samagam.markAsViewed', $t->id) }}" method="POST"
                                                        class="d-inline">
                                                        @csrf
